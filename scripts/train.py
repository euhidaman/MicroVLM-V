    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        try:
            if use_autocast:
                ...
                # Manual loss scaling for quantized models (prevents underflow)
                loss_scale = 2.0
        ...
        if use_autocast:
            ...
            # Manually unscale gradients before any further processing
        ...
        # Optimizer step (handles both autocast + quantized paths)
        if use_autocast:
...
def main():
    ...
    # Build dataloaders
    train_loader, val_loader, train_sampler = create_dataloaders(config, distributed=False)

    # Create model
    if getattr(config, 'alignment_mode', 'baseline') == 'fiber':
    ...
    scaler = GradScaler(enabled=not getattr(config, 'quantize_language_4bit', False))
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        print(f"Resumed from checkpoint {args.resume} at epoch {start_epoch}, step {global_step}")

    # Optional helpers
    visualizer = create_attention_visualizer(config) if getattr(config, 'enable_visualizations', False) else None
    attention_monitor = AttentionQualityMonitor(config) if getattr(config, 'use_attention_monitor', False) else None
    sliding_window_stopper = SlidingWindowEarlyStopping(
        ...
        patience_steps=getattr(config, 'sliding_window_patience', 3000),
        eval_interval=getattr(config, 'sliding_window_eval_interval', 100),
        verbose=True
    ) if getattr(config, 'use_sliding_window_early_stop', False) else None
    best_stage2_tracker = BestStage2ModelTracker(verbose=True) if getattr(config, 'track_best_stage2', False) else None
    carbon_tracker = None
        ...
        if stop_reason:
            ...
            if stop_reason == 'sliding_window_plateau' and stopper_status:
                push_stage2_final_to_hf(
                    checkpoint_path=str(Path(config.output_dir) / f"epoch_{epoch}_checkpoint.pt"),
                    config=config,
