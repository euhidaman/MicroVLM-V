...
class MicroVLM(nn.Module):
    ...
    def save_checkpoint(self, path):
        ...
        if self.quantize_4bit and hasattr(self.language_model, 'model'):
            # BitsAndBytes models should use save_pretrained instead
            # But for checkpoint compatibility, we'll save the state dict
            # The quantized weights are already in 4-bit format in memory
        ...
        if self.quantize_memory_158bit:
            ...
            for key in list(state_dict.keys()):
                ...
                if ('episodic_memory' in key or 'scope_detector' in key) and 'weight' in key:
                    # Skip if already quantized (e.g., from QuantizedLinear158BitGrad)
